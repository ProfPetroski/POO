public class Livro {
    ...
    public void addAutor(Autor autor){
        ...
        System.out.println("Não foi possível adicionar novo autor");
    }

    public void removeAutor(String nome){
        for(int i=0; i<this.autores.length; i++){
            if(this.autores[i]!=null&&this.autores[i].getNome()==nome){
                this.autores[i] = null;
                return;
            }
        }
        System.out.println("Não foi possível remover o autor: Nome não encontrado");
    }
    ...
    public String toString(){
        ...
        for(Autor a : this.autores){
            if(a!=null) {
                dadosLivro += "\t-------------------\n";
                dadosLivro += a.toString();
                dadosLivro += "\t-------------------\n";
            }
        }
        dadosLivro += "Número de páginas: \t"+this.numeroPaginas+"\n";
        dadosLivro += "Preço: \t"+this.preco+"\n";
